...
- **SAR: `sarColor` không áp cho chấm SAR vẽ trên chart**: `SARIndicator.drawChart` hard-code màu chấm theo trend (`candleStyle.upColor`/`dnColor`/`defaultTextColor`), bỏ qua `indicatorStyle.sarColor` — set `SARStyle(sarColor: ...)` qua `KChartColors.sarStyle` chỉ đổi được màu label `"SAR: ..."`, không đổi được chấm (khác mọi indicator khác, tự đọc `indicatorStyle.xxxColor` cho cả đường vẽ lẫn label). Sửa: chấm dùng thẳng `indicatorStyle.sarColor`.
