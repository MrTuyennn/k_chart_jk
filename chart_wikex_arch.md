...
| `nowPriceUpColor` / `nowPriceDnColor` | xanh / đỏ     | Đường + label giá hiện tại (`ChartPainter.drawNowPrice`) — so `livePrice ?? datas.last.close` với `open` nến cuối để chọn màu; xem [livePrice](#livePrice--cập-nhật-giá-real-time). |
